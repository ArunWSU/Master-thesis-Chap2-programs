...
class Plotstore:     
    def __init__(self,individual_plot_labels,fig_labels,plot_list,*args):
        ...
        self.save_plot_name=''
        self.X_scale=np.arange(1,plot_list[0].size+1,1)
        args_len=len(args)
        self.mark_select=1
        if(args_len==1):
        ...
        try:
            if(all(x.size==no_datapoints for x in self.plot_list)):
                    ...
                    for i in range(no_line_plots):
                         plt.plot(self.X_scale,self.plot_list[i],color=color_list[i],label=self.individual_plot_labels[i],linewidth=2,marker=marker_list[i],linestyle=line_style[i])
    ...
    def norm_fit(self,annual_data,plot_select):
